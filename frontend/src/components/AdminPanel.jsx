    ...
    <div style={{ position:'fixed', inset:0, zIndex:5000, background:'var(--white)', overflowY:'auto', animation:'slideFromLeft .5s cubic-bezier(.4,0,.2,1) both' }}>
      ...
      <div style={{ background:'var(--black)', color:'#fff', padding:'18px 44px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:10 }}>
        ...
        <div style={{ display:'flex', gap:12 }}>
          <button onClick={handleLogout} style={{ background:'none', border:'1px solid rgba(255,255,255,.25)', color:'rgba(255,255,255,.7)', padding:'7px 16px', borderRadius:4, fontSize:12, cursor:'none' }}>Logout</button>
          <button onClick={handleClose} style={{ background:'none', border:'none', color:'#fff', fontSize:22, cursor:'none' }}>✕</button>
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:0, borderBottom:'1px solid var(--border)', padding:'0 44px', background:'var(--white)', position:'sticky', top:58, zIndex:9 }}>
        {[['products','All Products'],['add','Add Product'],['security','Security']].map(([t,lbl]) => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            color: tab===t ? 'var(--black)' : 'var(--warm-gray)', cursor:'none', transition:'color .2s',
          }}>{lbl}</button>
        ))}
      </div>
        ...
        {tab === 'products' && (
          <div>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
              ...
              <button onClick={() => setTab('add')} style={{ padding:'10px 22px', background:'var(--black)', color:'var(--white)', border:'none', borderRadius:4, fontFamily:'var(--font-body)', fontSize:12, fontWeight:500, letterSpacing:'.1em', textTransform:'uppercase', cursor:'none' }}>+ Add Product</button>
            </div>
              ...
              {products.map(p => (
                <div key={p.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'13px 15px', border:'1px solid var(--border)', borderRadius:6, background:'var(--cream)' }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:12, color:'var(--warm-gray)' }}>{p.category} · Sizes: {p.sizes?.join(', ')} · {p.reviews?.length || 0} review{p.reviews?.length !== 1 ? 's' : ''}</div>
                  </div>
                  ...
                  <button onClick={() => handleDelete(p.id, p.name)}
                    style={{ background:'none', border:'1px solid var(--border)', color:'var(--warm-gray)', borderRadius:4, padding:'5px 11px', fontSize:11, cursor:'none', transition:'all .2s' }}
        ...
        {tab === 'add' && (
          <div>
            ...
            <button onClick={handleAdd} disabled={loading}
              style={{ padding:'13px 32px', background:'var(--black)', color:'var(--white)', border:'none', borderRadius:4, fontFamily:'var(--font-body)', fontSize:13, fontWeight:500, letterSpacing:'.1em', textTransform:'uppercase', cursor:'none' }}>
        ...
        {tab === 'security' && (
          <div style={{ maxWidth:520 }}>
            ...
            <button onClick={async () => {
              ...
              onToast?.('Password updated. Update backend/.env to persist ✔');
              setCurPw(''); setNewPw('');
            }} style={{ padding:'12px 28px', background:'var(--accent)', color:'#fff', border:'none', borderRadius:4, fontFamily:'var(--font-body)', fontSize:12, fontWeight:500, letterSpacing:'.08em', textTransform:'uppercase', cursor:'none' }}>
              Update Password
            </button>
          </div>
        )}
